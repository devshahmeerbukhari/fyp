...
type ChildProps = {
  onValueChange: (value: boolean, setIsModalOpen: boolean) => void;
};

function LoginAndRegister({ onValueChange }: ChildProps) {
  ...
  const checkLoginValidation = (event: React.FormEvent) => {
    ...
    if (!validation.success) {
      ...
      const user = ExistingUsers.find(
        u => u.email === loginFormData.email && u.password === loginFormData.password
      );
      if (user) {
        setLoginError({});
        onValueChange(true, false);
      } else {
        setLoginError({ email: ["Invalid credentials"] });
        onValueChange(false, true);
      }
    }
  };
